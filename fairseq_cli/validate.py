...
def main(cfg: DictConfig, override_args=None):
    ...
    for subset in cfg.dataset.valid_subset.split(","):
        ...
        fw_target = open(os.path.join(cfg.common_eval.results_path, "target.txt"), "w")
        fw_binder = open(os.path.join(cfg.common_eval.results_path, "binder.true.txt"), "w")
        fw_generation = open(os.path.join(cfg.common_eval.results_path, "binder.gen.txt"), "w")
        ...
        for i, sample in enumerate(progress):
            ...
            for idx, tgt in enumerate(target):
                ...
                for i in range(index):
                    target_protein.append(_srcs[idx][i])
                
                for i in range(index, len(_srcs[idx])):
                    binder_protein.append(_srcs[idx][i])
                
                for i in range(index, len(_strings[idx])):
                    gen_protein.append(_strings[idx][i])
